function runSimulation_Gridworld(lambda,P,num_memory_states,row,column,num_states,obs_function,T_horizon,num_trials,init,num_Obs)
...
for time=1:T_horizon
    for state=1:row*column
        combinedProbs(time,state) = sum(simulationMatrix(:,time,(time-1)*numStaNom+state))/num_trials;
    end
end
