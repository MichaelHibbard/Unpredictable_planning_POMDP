function [lambda,eta,a,b,slack1,cvx_optval]=CCCP_inner_problem4(tau,initLambda,initV,initEta,TF,transFuncReduced,absorb,absorbProduct,target,targetProduct,init,initReduced,unreachable,numMemory,cost,costLBound,obsFunction,discount)
...
variables lambda(numMemory,numActions,numObs) eta(numStates) slack1(numStates)
...
eta >= 0;       % Expected reward for reaching must be positive (this may be redundant?)
                        ...
                        for act = 1:numActions
                            ...
                            if transFuncReduced(staCounter,succ,act) ~= 0 && obsFunction(staCounter,obs) ~= 0
                                ...
                                if ~ismember(succ,absorbProduct) && ~ismember(succ,targetProduct)
                                    ...
                                    constantEta = discount*1/2*transFuncReduced(staCounter,succ,act)...
...
%                                     convexConcaveEtaTot = convexConcaveEtaTot + convexPartEta + concavePartEta;
                ...
                eta(staCounter) - probReachAccepting + sum_square(convexPartEta) + sum(concavePartEta) <= slack1(staCounter);
    ...
    maximize(eta(initReduced) - tau*sum(slack1));
    cvx_end
end
