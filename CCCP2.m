...
function [initial_lambda,initial_V,initial_eta]=CCCP(init_lambda,initial_V,init_eta,TF,TF_reduced,absorb,absorb_product, target, target_product,init,init_reduced,unreachable,num_memory,cost,cost_bound,obsFunction,discount)
    %%%%%%%% PARAMETERS
    ...
    for k=1:max_iter
        ...
        %[lambda,eta,V,slack_2,slack_1,optval]=CCCP_inner_problem3(tau,initial_lambda,initial_V,initial_eta,TF,TF_reduced,absorb,absorb_product, target, target_product,init,init_reduced,unreachable,num_memory,cost,cost_bound,obsFunction,discount);
        [lambda,eta,~,~,slack_1,optval]=CCCP_inner_problem4(tau,initial_lambda,initial_V,initial_eta,TF,TF_reduced,absorb,absorb_product, target, target_product,init,init_reduced,unreachable,num_memory,cost,cost_bound,obsFunction,discount);
        ...
        fprintf('Num iteration: %d,  Residual %0.5f,  Reach %0.4f \n', k, tau*(sum(slack_1)), eta(init_reduced))
%         disp(initial_lambda)
        if mod(k,25) == 0
...
%         if tau*(sum(slack_2)+sum(slack_1))<=epsilon && abs(optval-dummy_optval)<=10*epsilon
        if tau*(sum(slack_1))<=epsilon && abs(optval-dummy_optval)<=10*epsilon
            break;
        end
